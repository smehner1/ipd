...
function collect_netflow {
    ...
    # find ${COLLECTOR_LOCATION} -type d -exec chmod 755 {} \;
    # find ${COLLECTOR_LOCATION} -type f -exec chmod 644 {} \;

    # ----------------------------------------------------------------------------------------------------------------------
    #     preprocess all extracted filed     -------------------------------------------------------------------------------
    # ----------------------------------------------------------------------------------------------------------------------

    if [ $VERBOSE == 1 ]; then
        echo "|" -- preprocess collected netflow
    fi
    /home/max/WORK/masterthesis/miniconda3/envs/mini/bin/python3 /home/max/WORK/ipd-implementation/tools/preprocess_netflow.py -nf ${LOCAL_PATH} -outname ${COLLECTOR_LOCATION}/preprocessed_${SRC_FILE}
    exit_status=$?
    if [ $VERBOSE == 1 ]; then
