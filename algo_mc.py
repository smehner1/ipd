    ...
    def run(self):
        for current_ts in unique_ts:
            ...
            for i in cur_slice.itertuples():
                self.add_to_subnet(ip=i.src_ip, ingress=i.ingress, last_seen=i.ts_end)
            
            self.logger.info(f"current ts: {current_ts} ({len(cur_slice)} rows)")
...
if __name__ == '__main__':    
    ...
    print(f'read preprocessed netflow file: {args.d}')
    # netflow_df = dd.read_csv(args.d, blocksize=128*1024*1024, names=["src_ip","ts_end","ingress"])
    # netflow_df.ts_end = netflow_df.ts_end.apply(lambda x: int(int(x) / t) * t , meta=('ts_end', 'int64'))
    # netflow_df.compute()

    # netflow_df = pd.read_csv(args.d, chunksize=500000, low_memory=False, names=["src_ip","ts_end","ingress"])
    ...
    if True:
        length_mapping= {"/data/slow/mehner/netflow.csv": 2063550166,
                        "/data/slow/mehner/netflow100000.csv" : 100000,
                        "/data/slow/mehner/netflow100000000.csv": 100000000
                        }
        ...
        testi_mesti = list(pd.read_csv(args.d , chunksize=chunk_size, low_memory=False, names=["src_ip","ts_end","ingress"]))
        pool = Pool(processes=PROCS)

        def read_csv_parallel_worker(chunk):
            print(chunk)

            return pd.DataFrame(chunk)
            

        res= pool.imap(read_csv_parallel_worker, testi_mesti)

        for a in res:
            netflow_df = pd.concat([netflow_df, a])
            print("join, ", a)

        pool.close()
        pool.join()
        # with tqdm(total=length_mapping.get(args.d, 2063550166), desc="chunks read: ") as bar:
        # # enumerate chunks read without low_memory (it is massive for pandas to precisely assign dtypes)
        #     for i, chunk in enumerate(pd.read_csv(args.d , chunksize=chunksize, low_memory=False, names=["src_ip","ts_end","ingress"])):
        ...
        print(now)
        netflow_df = pd.read_parquet("/data/slow/mehner/netflow.parquet")
        netflow_df.columns = ["src_ip","ts_end","ingress"]
        netflow_df = netflow_df.convert_dtypes()

        
        print("... done  took: {}".format(datetime.datetime.now))
        print(netflow_df.head())
    
    netflow_df['ts_end'] = netflow_df.ts_end.apply(lambda x: int(int(x) / t) * t) 
    
    unique_ts= sorted(netflow_df.ts_end.unique())
    print(unique_ts)
    ...
    if TEST:
        ...
        param_list=[
            ...
            # expire time
            params(t, b, 30, 0.95,  64, 24,28, 48, 'default', logging.INFO),
            params(t, b, 300, 0.95, 64, 24,28, 48, 'default', logging.INFO),

            # q
            params(t, b, 120, 0.85, 64, 24,28, 48, 'default', logging.INFO),
            params(t, b, 120, 0.75, 64, 24,28, 48, 'default', logging.INFO),
            params(t, b, 120, 0.65, 64, 24,28, 48, 'default', logging.INFO),
            params(t, b, 120, 0.55, 64, 24,28, 48, 'default', logging.INFO),

            # cidrmax
            params(t, b, 120, 0.95, 64, 24,26, 44, 'default', logging.INFO),
            params(t, b, 120, 0.95, 64, 24,24, 40, 'default', logging.INFO),
            params(t, b, 120, 0.95, 64, 24,20, 36, 'default', logging.INFO),

            # decay
            params(t, b, 120, 0.95, 64, 24,28, 48, 'linear', logging.INFO),
            params(t, b, 120, 0.95, 64, 24,28, 48, 'stefan', logging.INFO),
            ...
            # q
            params(t, b, 120, 0.85, 4, 1,28, 48, 'default', logging.INFO),
            params(t, b, 120, 0.75, 4, 1,28, 48, 'default', logging.INFO),
            params(t, b, 120, 0.65, 4, 1,28, 48, 'default', logging.INFO),
            params(t, b, 120, 0.55, 4, 1,28, 48, 'default', logging.INFO),

            # cidrmax
            params(t, b, 120, 0.95, 4, 1,26, 44, 'default', logging.INFO),
            params(t, b, 120, 0.95, 4, 1,24, 40, 'default', logging.INFO),
            params(t, b, 120, 0.95, 4, 1,20, 36, 'default', logging.INFO),

            # decay
            params(t, b, 120, 0.95, 4, 1,28, 48, 'linear', logging.INFO),
