...
TIME_LOG = '/home/max/WORK/ipd-implementation/log/times.txt'
...
if MINI_SETUP:
    router_ip_mapping_file = "router_lookup_tables/mini-internet.txt"  # {'ip': 'routername'}
    ingresslink_file = "ingresslink/mini-internet.gz"                # if we get more netflow, we should adjust the file
else:
    ingresslink_file = "ingresslink/1605571200.gz"                # if we get more netflow, we should adjust the file
    router_ip_mapping_file = "router_lookup_tables/1605571200.txt"


# netflow_path="/data/slow/mehner/netflow/dummy_netflow.gz"
# ingresslink_file = "ingresslink/1605571200.gz"                # if we get more netflow, we should adjust the file
# ingresslink_file = "ingresslink/mine.gz"                # if we get more netflow, we should adjust the file
# router_ip_mapping_file="/data/slow/mehner/ipd/router_lookup_tables/1605571200.txt"
# router_ip_mapping_file = "router_lookup_tables/1605571200.txt"
# router_ip_mapping_file = "router_lookup_tables/mine.txt"  # {'ip': 'routername'}
...
class IPD:
    ...
    def __init__(self, params):
        ...
        # TODO: permission denied error --> solve by running script with sudo
        if DUMP_TREE:
        ...
        if count < 0:
            try:
                ...
                for masked_ip in self.subnet_dict[ip_version][mask][prange]:
                    count += self.subnet_dict.get(ip_version,
                                                  {}).get(mask, {}).get(prange, {}).get(masked_ip, {}).get('total', 0)
            ...
            # not classified yet
                ...
                while len(masked_ips_list) > 0:
                    ...
                    for ingress in list(self.subnet_dict[ip_version][mask][prange][masked_ip]['ingress'].keys()):
                        counter_dict[ingress] += self.subnet_dict[ip_version][mask][prange][masked_ip]['ingress'][
                                                    ingress]
    ...
    def join_siblings(self, ip_version, mask, prange, current_ts, counter_check=True):
        ...
        self.remove_expired_ips_from_range(
            ip_version,
            int(the_other_one.split('/')[1]),
            the_other_one.split('/')[0], current_ts
        )
        ...
        if (tmp_cur_prevalent is not None) or (tmp_merged_sample_count < min_samples and counter_check):
            ...
            self.logger.debug("len before: {}".format(len(list(
                self.subnet_dict[ip_version][supernet_mask][supernet_ip].keys()))))
            ...
            for sibling in siblings:
                ...
                self.logger.debug("{} -> {}".format(sibling, len(
                    self.subnet_dict[ip_version][mask][str(sibling).split("/")[0]])))
                ...
                try:
                    ...
                    # self.logger.debug("   {}".format(self.range_lookup_dict[ip_version]))
                    pass
            self.logger.debug("len now: {}".format(len(list(
                self.subnet_dict[ip_version][supernet_mask][supernet_ip].keys()))))
    ...
    def add_to_subnet(self, masked_ip, ingress, last_seen, i_count=1):
        ...
        if p_ingress is None:  # 1) no prevalent ingress found for that range
            ...
            if self.debug_flow_output_counter > DEBUG_FLOW_OUTPUT:
                self.logger.debug((f"  not classified yet - "
                                   f"{self.subnet_dict[int(ip_version)][int(mask)][prange][masked_ip]}"))
                pass
            ...
            # 2a) there is one single prevalent link
            elif ingress == p_ingress:
                # do nothing since we already incremented totals
                pass
            else:
                self.subnet_dict.get(ip_version, {}).get(mask, {}).get(prange, {})['miss'] += i_count
                if self.debug_flow_output_counter > DEBUG_FLOW_OUTPUT:
                    self.logger.debug((f"  already classified but ingress not correct - "
                                       f"{self.subnet_dict.get(ip_version, {}).get(mask,{}).get(prange,{})}"))
                    pass
    ...
    def __decay_counter(self, ip_version, mask, prange, current_ts, method="none"):  # default, linear, stefan
        ...
        self.logger.debug((f"current_ts: {current_ts}; e: {self.e}; last_seen: {last_seen}; age:{age} total: {totc} "
                          f"miss: {misc} "))
        reduce = 0
        if method == 'default':  # ingmar
            ...
            #   return 1 - (($age <= 0) ? $bucketExpireKeepFraction : ($bucketExpireKeepFraction/(int($age/$bucketSize)
            #                   +1)));
            # }
            # my $reduce = int($cidrIntRef->{ip}{$color}{$ipInt}{c} * (getCleanKeepFactor($expireTime, $cidrIntRef
            #   ->{ip}{$color}{$ipInt}{u})));
        ...
        if self.subnet_dict[ip_version][mask][prange].get("prevalent_last_seen", None) is not None:
            ...
            while len(masked_ip_list) > 0:
                ...
                try:
                    ...
                    self.logger.warning((f"last seen not here: {masked_ip} "
                                         f"{self.subnet_dict[ip_version][mask][prange][masked_ip]}"))
                ...
                if last_seen < current_ts - self.e:
                    ...
                    for ingress in self.subnet_dict[ip_version][mask][prange][masked_ip]['ingress'].keys():
                        counter_dict[ingress] += self.subnet_dict[ip_version][mask][prange][
                            masked_ip]['ingress'][ingress]
                    pass
                    ...
                    for mask in self.subnet_dict[ip_version].keys():
                        for prange in self.subnet_dict[ip_version][mask].keys():
                            ...
                            if min_samples > total_samples:
                                self.logger.warning((f"total count lower than min samples: {total_samples} / "
                                                     f"{min_samples}"))
    ...
    def run_ipd(self, current_ts):
        ...
        if DUMP_TREE and (current_ts % dump_output == 0):
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # print(self.netflow_data_dict.keys())
                # print(len(self.netflow_data_dict.keys()))
                if (len(self.netflow_data_dict.keys()) < 2) and (-1 not in self.netflow_data_dict.keys()):
                    if MINI_SETUP:
                        self.logger.info("dict empty - wait 20s")
                        time.sleep(20)
                    else:
    ...
    def read_netflow(self):
        ...
        for line in sys.stdin:
            ...
            # print("{}.{}".format(router_name,in_iface))

            if line[-3] == "TIMESTAMP_END":
                continue

            # print(ingresslink_dict)
            # print(f"{router_name}.{in_iface}")
            if not ingresslink_dict.get(f"{router_name}.{in_iface}", False):
                # print('BAD')
                continue
            src_ip = line[4]
            # cur_ts = int(float(line[-3]) / self.t) * self.t
            # print()
            # print(self.t)
            # print(float(line[-3]))
            # print(float(line[-3]) / self.t)
            # print(int(float(line[-3]) / self.t))
            # if MINI_SETUP:
            # cur_ts = int(line[-3])
            # else:
            # print(int(int(line[-3]) / self.t))
            cur_ts = int(float(line[-3]) / self.t) * self.t
...
if __name__ == '__main__':
    ...
    # parser.add_argument('-d', default="/data/fast/mehner/ipd/netflow_merged_sorted", type=str) # netflow100000.csv
    # netflow100000000.csv
    parser.add_argument('-d', default="/test/", type=str)  # netflow100000.csv netflow100000000.csv
