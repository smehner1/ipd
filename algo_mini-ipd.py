...
from tools import connect_netflow
...
savepoints = [1605556860, 1605560460, 1605564060, 1605567660, 1605571260, 1605574860, 1605578460, 1605582060,
              1605585660, 1605589260, 1605592860, 1605596460, 1605600060, 1605603660, 1605607260, 1605610860,
              1605614460, 1605618060, 1605621660, 1605625260, 1605628860, 1605632460, 1605636060, 1605639660,
              1605643260]
...
cols = ['tag', 'peer_src_ip', 'in_iface', 'out_iface', 'src_ip', 'dst_net', 'src_port', 'dst_port', 'proto', '__', '_',
        'ts_start', 'ts_end', 'pkts', 'bytes']
...
ingresslink_file = f"{base_path}/ingresslink/mini-internet.gz"  # if we get more netflow, we should adjust the file
# router_ip_mapping_file = f"{base_path}/router_lookup_tables/1605571200.txt"
router_ip_mapping_file = f"{base_path}/router_lookup_tables/mini-internet.txt"
...
class IPD:
    ...
    def __init__(self, params):
        ...
        self.output_folder = (f"{base_path}/algo/results/{RESULT_PREFIX}/q{self.q}_c{self.c[4]}-{self.c[6]}_cidr_max"
                              f"{self.cidr_max[4]}-{self.cidr_max[6]}_t{self.t}_e{self.e}_decay{self.decay_method}")
        self.tree_output_folder = (f"{base_path}/algo/dump/{RESULT_PREFIX}/q{self.q}_c{self.c[4]}-{self.c[6]}_cidr_max"
                                   f"{self.cidr_max[4]}-{self.cidr_max[6]}_t{self.t}_e{self.e}_decay"
                                   f"{self.decay_method}")
        if TEST:
        ...
        self.resource_logfile = (f"{base_path}/algo/resource_log/{RESULT_PREFIX}/q{self.q}_c{self.c[4]}-{self.c[6]}"
                                 f"_cidr_max{self.cidr_max[4]}-{self.cidr_max[6]}_t{self.t}_e{self.e}"
                                 f"_decay{self.decay_method}")
        if TEST:
        ...
        logfile = (f"{base_path}/algo/log/{RESULT_PREFIX}/q{self.q}_c{self.c[4]}-{self.c[6]}_cidr_max"
                   f"{self.cidr_max[4]}-{self.cidr_max[6]}_t{self.t}_e{self.e}_decay{self.decay_method}")
        if TEST:
            logfile += "_TEST"
        logfile += ".log"
    ...
    def init_resource_consumption_logfile(self):
        ...
        with open(self.resource_logfile, fmode, newline='') as csvfile:
            ...
            writer.writerow(
                ['ts', 'ipd_ranges_count', 'ipd_cpu_runtime', 'iteration_cpu_runtime', 'ipd_runtime',
                 'iteration_runtime', 'ram_usage', 'shared_ram_usage', 'total_ram', 'avail_ram']
                )

    def log_resource_consumption(
            self, cur_ts, range_count, ipd_cpu_runtime, iteration_cpu_runtime, ipd_runtime, iteration_runtime,
            ram_usage, ram_shared, ram_total, ram_avail):

        with open(self.resource_logfile, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',')
            writer.writerow(
                [cur_ts, range_count, ipd_cpu_runtime, iteration_cpu_runtime, ipd_runtime, iteration_runtime, ram_usage,
                 ram_shared, ram_total, ram_avail]
                )
    ...
    def get_sample_count(self, ip_version, mask, prange):
        ...
        if count < 0:
            try:
                count = sum(
                    self.ipd_cache.get(int(ip_version), {}).get(int(mask), {}).get(prange, {}).get('cache', -1).values()
                )
            except Exception as e:
                ...
                for masked_ip in self.subnet_dict[ip_version][mask][prange]:
                    count += self.subnet_dict.get(
                        ip_version, {}).get(mask, {}).get(prange, {}).get(masked_ip, {}).get('total', 0)
    ...
    def check_if_enough_samples_have_been_collected(self, ip_version, mask, prange):
        ...
        self.logger.info((f"  > Check if enough samples have been collected (s_ipcount >= n_cidr ) {ip_version} {mask} "
                          f"{prange}  s_ipcount={sample_count} min_samples={min_samples}"))
    ...
    def get_prevalent_ingress(self, ip_version, mask, prange, raw=False):
        ...
        def __get_prev_ing(counter_dict):
            ...
            if prevalent_ingress is None:  # still no prevalent ingress? -> check for bundles
                ...
                for ingress in result_dict.keys():
                    ...
                    if value + b >= last_value:
                        ...
                        elif (len(bundle_candidates) > 0
                              and (list(bundle_candidates)[0].split(".")[0]) == ingress.split(".")[0]):
                            bundle_candidates.add(ingress)
                ...
                if prevalent_ingress is None:
                    ratio = -1, 00
                # self.ipd_cache[ip_version][mask][prange]['cache_prevalent_ingress']  = prevalent_ingress
                # self.ipd_cache[ip_version][mask][prange]['cache_prevalent_ratio'] = ratio

            self.logger.debug((f"        prevalent for {ip_version} {mask} {prange}: {prevalent_ingress} "
                               f"({prevalent_ratio:.2f})"))
        ...
        if len(counter_dict) > 0:
            ...
            if p_ingress is not None and p_total is not None:  # there is a prevalent ingress yet
                if p_total < 1:
                    ...
                    self.logger.info((f"p_total < 1: {ip_version} {mask} {prange} ingress:{p_ingress} total:{p_total} "
                                      f"miss:{p_miss} - pop: {pr}"))
                ...
                if res is None:
                    self.logger.warning((f"        prevalent ingress {p_ingress} for {ip_version} {mask} {prange} "
                                         f"below threshold ({ratio}) (will be popped in another step)"))
            ...
            # not classified yet
                ...
                while len(masked_ips_list) > 0:
                    ...
                    for ingress in list(self.subnet_dict[ip_version][mask][prange][masked_ip]['ingress'].keys()):
                        counter_dict[ingress] += self.subnet_dict[
                                                    ip_version][mask][prange][masked_ip]['ingress'][ingress]
    ...
    def set_prevalent_ingress(self, ip_version, mask, prange, ingress, current_ts):
        ...
        if type(ingress) == list:  # bundle
            self.bundle_id += 1
            # name of bundle
            prevalent_name = "{}{}{}".format(ingress[0].split(".")[0], bundle_indicator, self.bundle_id)
        else:  # single
        ...
        self.logger.info((f"        set prevalent ingress: {ip_version} {mask} {prange} => {prevalent_name}: "
                          f"{ip_version} range {ratio:.3f} {sample_count}/{min_samples} {prange}/{mask} "
                          f"{prevalent_name} | miss: {miss} total: {sample_count}"))
    ...
    def is_prevalent_ingress_still_valid(self, ip_version, mask, prange, current_ts):
        ...
        if ((current_prevalent == new_prevalent) or ((type(new_prevalent) == list) and
            (bundle_indicator in current_prevalent) and
             (list(self.bundle_dict.get(current_prevalent).keys()).sort() == sorted(new_prevalent)))):
    ...
    def split_range(self, ip_version, mask, prange):
        ...
        self.subnet_dict[ip_version][mask].pop(prange)
        # self.logger.info(f"range_lookup_dict: (elem count: {len(list(self.range_lookup_dict[ip_version])[:10])})
        #   {list(self.range_lookup_dict[ip_version])[:10]}")
    ...
    def join_siblings(self, ip_version, mask, prange, current_ts, counter_check=True):
        ...
        for sibling in siblings:
            ...
            if self.range_lookup_dict[ip_version].get(str(sibling), None) is None:
                self.logger.debug(f"sibling does not exist: {str(sibling)} - abort joining")
                # self.logger.info(f"range_lookup_dict: (elem count: {len(list(self.range_lookup_dict[ip_version]))})
                #   {list(self.range_lookup_dict[ip_version])[:10]}")
                return None
        ...
        self.remove_expired_ips_from_range(
            ip_version, int(the_other_one.split('/')[1]), the_other_one.split('/')[0], current_ts)
        ...
        s1 = s1 if s1 is not None else {}
        s2 = s2 if s2 is not None else {}

        if (len(s1) == 0 and len(s2) == 0):
            self.logger.debug("both prefixes have no prevalent ingress")

        #     self.logger.warning(f"        both prefixes are empty - pop siblings and create supernet
        #           {str(nw_supernet)}")
        ...
        if (tmp_cur_prevalent is not None) or (tmp_merged_sample_count < min_samples and counter_check):
            ...
            self.logger.debug(
                "len before: {}".format(len(list(self.subnet_dict[ip_version][supernet_mask][supernet_ip].keys()))))
            ...
            for sibling in siblings:
                ...
                try:
                    ...
                    pass
            self.logger.debug(
                "len now: {}".format(len(list(self.subnet_dict[ip_version][supernet_mask][supernet_ip].keys()))))
    ...
    def add_to_subnet(self, masked_ip, ingress, last_seen, i_count=1):
        ...
        #   2a) there is one single prevalent link:      -> increment total and increment miss if it is not the
        #                                                   correct ingress
        #   2b) there is a prevalent bundle:             -> increment total and increment miss in self.subnet_dict AND
        #                                                   increment matches for ingresses in bundle dict
        ...
        if self.debug_flow_output_counter > DEBUG_FLOW_OUTPUT:
            self.logger.debug((f"add flow {masked_ip}, {ingress}, {last_seen} --> {ip_version}, {mask}, {masked_ip} "
                               f"--> {self.get_corresponding_range(masked_ip)}"))
            self.debug_flow_output_counter = 0

        # get current prev ingress if existing
        p_ingress = self.subnet_dict.get(ip_version, {}).get(mask, {}).get(prange, {}).get('prevalent', None)

        if p_ingress is None:  # 1) no prevalent ingress found for that range
            ...
            if self.debug_flow_output_counter > DEBUG_FLOW_OUTPUT:
                self.logger.debug(
                    f"  not classified yet - {self.subnet_dict[int(ip_version)][int(mask)][prange][masked_ip]}")
                pass
            ...
            if (bundle_indicator in p_ingress) and (ingress in self.bundle_dict[p_ingress].keys()):
                ...
                if self.debug_flow_output_counter > DEBUG_FLOW_OUTPUT:
                    self.logger.debug(
                        "  already classified but ingress not correct - {}".format(
                            self.subnet_dict.get(ip_version, {}).get(mask, {}).get(prange, {})
                        )
                    )
                    pass
    ...
    def __decay_counter(self, ip_version, mask, prange, current_ts, method="none"):  # default, linear, stefan
        ...
        if method == 'default':  # ingmar
            # my $bucketExpireKeepFraction = 0.9; # all counts get decreased to this fraction every time a bucket is
            #   flushed. This fraction decreases for stale buckets
            # $cidrIntRef->{ip}{$color}{$ipInt}{u} => $lastUpdate
            #
            # sub getCleanKeepFactor{
            #   (my $expireTime, my $lastUpdate) = @_;
            #   my $age = $expireTime - $lastUpdate;
            #   return 1 - (($age <= 0) ? $bucketExpireKeepFraction :
            #       ($bucketExpireKeepFraction/(int($age/$bucketSize)+1)));
            # }
            # my $reduce = int($cidrIntRef->{ip}{$color}{$ipInt}{c} * (getCleanKeepFactor($expireTime,
            #       $cidrIntRef->{ip}{$color}{$ipInt}{u})));
    ...
    def remove_expired_ips_from_range(self, ip_version, mask, prange, current_ts):
        '''
            ...
            check if there is the attribute 'prevalent_last_seen'
                ...
                -> N: iterate over all underlying (masked) IPs and check every single IP if it is expired
                    (OPTIMIZATION: after removing the IPs, we calc current prevalent ingress and append it to the
                                   masked_ip as attribute)
        ...
        if self.subnet_dict[ip_version][mask][prange].get("prevalent_last_seen", None) is not None:
            ...
            self.logger.info((f"decay {ip_version} {mask} {prange}, {current_ts}: total before: {total_before}; total "
                              f"now: {total_now}"))
            ...
            self.logger.debug((f"count before: {count_counter} - expired {pop_counter} = now: "
                               f"{sum(counter_dict.values())} check: {count_counter-pop_counter}"))
                ...
                for ip_version in self.subnet_dict.keys():
                    for mask in self.subnet_dict[ip_version].keys():
                        for prange in self.subnet_dict[ip_version][mask].keys():
                            ...
                            self.logger.info((f"{current_ts}\t{ip_version}\trange\t{ratio:.3f}\t{total_samples}/"
                                              f"{min_samples}\t{prange}/{mask}\t{ingress}"))
                            encode.write((f"{current_ts}\t{ip_version}\trange\t{ratio:.3f}\t{total_samples}/"
                                          f"{min_samples}\t{prange}/{mask}\t{ingress}\n"))
    ...
    def run_ipd(self, current_ts):
        ...
        self.logger.warning((f"............. run IPD {current_ts}  -> {len(check_list)} elems  RAM: "
                             f"{current_ram_usage_bytes} ............."))
        mem = psutil.virtual_memory()

        self.logger.warning((f"{current_ts} RAM:{current_ram_usage_bytes} mem_process_rss:"
                             f"{self.process.memory_full_info().rss} mem_process_shared:"
                             f"{self.process.memory_full_info().shared} mem_node_total:{mem.total} "
                             f"mem_node_available:{mem.available}"))
        ...
        while len(check_list) > 0:
            ...
            if self.subnet_dict[ip_version][mask][prange].get('prevalent', None) is not None:
                ...
                if r is True:
                    ...
                    if prevalent_ingress is not None:
        ...
        for line in stdin:
            ...
            if ram_counter >= RAM_CHECK_AFTER_N_LINES:
                ...
                if psutil.virtual_memory()[2] > RAM_THRESHOLD:
                    self.logger.warning((f"RAM WARNING: currently {psutil.virtual_memory()[2]} in use -> cooldown for "
                                         f"{RAM_COOLDOWN_TIME} seconds"))
            ...
            # line = line.split(col_mapping.get(','))
            ...
            # print(type(line[12]))
            try:
                cur_ts = int(float(line[col_mapping.get('ts_end')]) / self.t) * self.t
                # print(cur_ts)
...
if __name__ == '__main__':
    ...
    # netflow100000.csv netflow100000000.csv
    parser.add_argument('-d', default="/data/fast/mehner/ipd/netflow_merged_sorted", type=str)
